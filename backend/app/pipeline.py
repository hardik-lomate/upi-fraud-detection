...
from feature_contract import MODEL_VERSION, FEATURE_COLUMNS, get_risk_tier, RISK_TIER_MAP
...
class PipelineContext:
    ...
    # Step 6: User warning payload (pre-check UX)
...
def step_extract_features(ctx: PipelineContext, extract_fn) -> PipelineContext:
    ...
    try:
        ...
        # Backward compatibility: if extractor returns an older but valid subset,
        # fill newly added contract columns with neutral defaults.
        for col in FEATURE_COLUMNS:
            if col not in ctx.features:
...
def step_decide(ctx: PipelineContext) -> PipelineContext:
    ...
    from .decision_engine import make_decision, build_reasons, compute_personalized_threshold
...
def step_user_warning_payload(ctx: PipelineContext) -> PipelineContext:
    """Step 6: Build user-facing warning payload for pre-payment interception."""
...
def run_pipeline(
    ...
    validate → features → rules → ML → device → graph → decide → explain → user_warning
